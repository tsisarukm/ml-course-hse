...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    unique_features, counts = np.unique(sorted_feature, return_counts=True)
    ...
    p_l = prefix_sum[:-1] / Rl
    p_r = (prefix_sum[-1] - prefix_sum[:-1]) / Rr
    Hl = 1 - p_l ** 2 - (1 - p_l) ** 2
    Hr = 1 - p_r ** 2 - (1 - p_r) ** 2
    ginis = -(Rl / R) * Hl - (Rr / R) * Hr

    valid_indices = np.cumsum(counts)[:-1] - 1
...
class DecisionTree:
    ...
    def _fit_node(self, sub_X, sub_y, node, cur_depth):
        ...
        if ((self._max_depth and cur_depth >= self._max_depth) or (
                self._min_samples_split and len(sub_X) < self._min_samples_split) \
                or (self._min_samples_leaf and len(sub_X) < 2 * self._min_samples_leaf)):
            node["type"] = "terminal"
    ...
    def _predict_node(self, x, node):
        ...
        if self._feature_types[feature_to_split] == 'real':
            if x[feature_to_split] < node["threshold"]:
                return self._predict_node(x, node["left_child"])
            else:
                return self._predict_node(x, node["right_child"])
        elif self._feature_types[feature_to_split] == 'categorical':
            categorical_feature = node["categories_split"]
            if x[feature_to_split] in categorical_feature:
                return self._predict_node(x, node['left_child'])
            else:
                return self._predict_node(x, node['right_child'])
